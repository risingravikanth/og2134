 angular.module('OGAnalysis').controller('SmallScaleCtrl', function($scope,$state,$rootScope,URL,HttpService) {
	console.log("In SmallScaleCtrl ctrl");
 	console.log($state);
 	
 	$scope.setConfigurations = function(){
 		$scope.url = "/smallscalelng/asset";
 		$rootScope.loadSmallScaleFilter();
		$rootScope.filterObj = {
			regionField :true,
			countryField :true,
			locationField : true,
		...
		$scope.columns = [
			...
			{ title: "Company",
	 		...
	 		{ title: "Liquefaction Capacity(Tonnes/day)",
			  data: "liquefactionCapacity"
			},
	 		{ title: "Regasification Capacity(MMcf/day)",
				  data: "regasificationCapacity"
				},
		 		{ title: "Storage Capacity(m3)",
